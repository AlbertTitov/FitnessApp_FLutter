...
import 'package:fitness_app/components/drill/drill-remove-alert.dart';
import 'package:fitness_app/components/drill/drill.dart';
...
class _AddWorkoutDayState extends State<AddWorkoutDay> {
  ...
  var _drillTypeMenuItems = <String> [
    ...
    'AMRAP',
    'EMOM'
  ].map((String value) => DropdownMenuItem<String>(value: value,
      child: Text(value)))
      .toList();
  ...
  void _saveDayPlan() {
    if (_fbKey.currentState.saveAndValidate()) {
      if (day.drillBlocks.length > 0) {
        print(day.drillBlocks
            .where((d) => d is WorkoutRestDrillBlock)
            .length);
        if (day.drillBlocks
            .where((d) => d is WorkoutRestDrillBlock)
            .length == day.drillBlocks.length) {
          buildToast('Please add at least onr drill');
          return;
        }
      }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('FitnessApp // Create Day Plan'),
        actions: <Widget>[
      ...
      child: Card(
        ...
        child: Container(
          decoration: BoxDecoration(
            color: _getDrillBlockHeaderColor(context, block)
          ),
          child: Column(
            children: <Widget>[
              ListTile(
                ...
                title: Row(children: <Widget>[
                  ...
                  _canSelectDrillsCount(block) ?
                      DropdownButton(
                        items: Iterable<int>.generate(5).map((val) => DropdownMenuItem(
                          value: _isMultisetWorkout(block) ? val + 2 : val + 1,
                          child: Text('${ _isMultisetWorkout(block) ? (val + 2) : (val + 1) } drills'),
                        )).toList(),
                        onChanged: (val) {
                          setState(() {
                ...
                trailing: IconButton(
                  ...
                  color: Colors.red,
                  onPressed: () async {
                    var result = await showDialog(context: context, builder: (_) => drillRemoveAlert(context));
  ...
  Widget _buildDrills(int blockIndex, WorkoutDrillsBlock block) {
    return Column(
      children: Iterable<int>.generate(block.drills.length).map((index) => Drill(isSingleDrill: block.drills.length <= 1,
          drillBlockIndex: blockIndex,
          index: index,
          drill: block.drills[index])).toList()
    );
  }
  ...
  Color _getDrillBlockHeaderColor(BuildContext context, WorkoutDrillsBlock block) {
  ...
  bool _isMultisetWorkout(WorkoutDrillsBlock block) => block is WorkoutMultisetDrillBlock;
}